...
Reviewed 2014-06-10: Made all the {\tt -Wall}s go away.\\
Reviewed 2013-11-24.
...
\begin{code}
   fx c a rs cps = case cps of
      ...
      ((c',p):cps')
         | c == c'   -> a ((c',p):rs) cps'
         | otherwise -> reverse rs ++ [(c',p)] ++ f cps'
\end{code}
...
\begin{code}
octitL :: Lexer
octitL = satisfyL isOctDigit "octit"
\end{code}
...
varidL = 
   ...
   (\(((_,ident),_):_) -> ident `notElem` reservedids)
\end{code}
...
varsymL = 
   ...
   (\(((_,sym),_):_) -> sym `notElem` reservedops)
\end{code}
...
\begin{code}
importD :: TLPs -> TLPs
importD ((("varid","qualified"),_):tlps) = importD tlps
...
dropContext (_:tlps) = dropContext tlps
\end{code}
